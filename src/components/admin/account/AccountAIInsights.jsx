...
import { generateAccountInsights } from './accountInsights'
...
export default function AccountAIInsights({ context }) {
  const insights = generateAccountInsights(context)

  return (
    <section className="acc-glass-card p-4">
      <header className="mb-3 flex items-center gap-2">
        <motion.span
          className="flex h-8 w-8 items-center justify-center rounded-xl border border-violet-400/30 bg-violet-500/15"
          animate={{ boxShadow: ['0 0 0 rgba(139,92,246,0)', '0 0 20px rgba(139,92,246,0.25)', '0 0 0 rgba(139,92,246,0)'] }}
        ...
        <motion.div animate={{ opacity: [0.85, 1, 0.85] }} transition={{ duration: 4, repeat: Infinity }}>
          ...
          <h3 className="text-sm font-semibold text-slate-100">Identity intelligence</h3>
        </motion.div>
      </header>
      <ul className="space-y-2">
        {insights.map((item, i) => (
          <motion.li
            ...
            whileHover={{ x: 2 }}
            className={`rounded-xl border px-3 py-2.5 transition-shadow hover:shadow-[0_0_24px_rgba(99,102,241,0.08)] ${TONE[item.tone] || TONE.sky}`}
          >
