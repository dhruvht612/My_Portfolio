...
function ShaderCanvas({ fragSource, pixelRatio }) {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn('[ShaderGridBackground] WebGL2 not available')
      return undefined
    }
    ...
    function tick(now) {
      ...
      const t = reduceMotion ? 0 : (now - startRef.current) / 1000
      frameRef.current += 1
...
/**
 * Animated navy mesh + grid WebGL2 background. Sits under particles / glass overlay.
